...
class ESAU(nn.Module):
    def __init__(self,opt,in_channels=1,out_channels=1,n_channels=64,num_heads=[1,2,4,8],res=True):
        ...
        self.out2 = SingleConv(n_channels//2,out_channels,res=res,activation=True)
...
if __name__ == "__main__":
    firstU = ESAU("Converter",in_channels=5,n_channels=256,out_channels=1)
    img_channels = 5
    img_size = 256
    x = torch.randn((3, img_channels, img_size, img_size))
    output = firstU(x)
    print(output.shape)
